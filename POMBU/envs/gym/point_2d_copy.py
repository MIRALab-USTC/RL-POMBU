...
class PointEnv(gym.Env):
    ...
    def __init__(self, init_sampling_boundaries=(-2,2), goal=(0, 0), **kwargs):
        self.init_sampling_boundaries=init_sampling_boundaries
        self.goal = np.array(goal)
        self.action_space = spaces.Box(low=np.array([-0.1, -0.1]), high=np.array([0.1, 0.1]))
    ...
    def _reset(self):
        self._state = self.np_random.uniform(self.init_sampling_boundaries[0], self.init_sampling_boundaries[1], size=(2,))
    ...
    def estimate_V(self, state, pi, num_traj=20, horizon=30, gamma=0.99):
        V = 0
        state = np.array([state] * num_traj).reshape(-1,2)
        a = 1
    ...
    def estimate_Q(self, state, action, pi, num_traj=20, horizon=30, gamma=0.99):
        Q = 0
        state = np.array([state] * num_traj).reshape(-1,2)
        action = np.array([action] * num_traj).reshape(-1,2)
        a = 1
        for _ in range(horizon):
            _state = state + np.clip(action, -0.1, 0.1)
            r = self.reward(state, action, _state)
