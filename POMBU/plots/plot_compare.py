...
colors = ["orangered", "darkorange", "lightseagreen", "cornflowerblue", "orchid", "gray", "darkseagreen", "goldenrod", "darkorange", "mediumorchid", "darkturquoise" ]
COLORS = dict()
...
def get_data_from_algo_dir(algo_dir, algo_name, x_limit):
    ...
    return compute_mean_std_max(plot_x, plot_y)
...
def compute_mean_std_max(plot_x, plot_y):
    ...
    max_index = np.argmax(y_mean)
    return x, y_mean, y_std, x[max_index], y_mean[max_index], y_std[max_index]
    ...
    for env_name in os.listdir(log_dir):
        ...
        for algo_name in os.listdir(env_dir):
            ...
            if algo_name in algo_dic:
                ...
                continue
            x, y_mean, y_std, max_x, max_y_mean, max_y_std = get_data_from_algo_dir(algo_dir, algo_name, x_limit)
            plot_data.append((LEGEND_ORDER[algo_name], algo_name, x, y_mean, y_std, max_y_mean))
            print(env_name, algo_name)
            for x, mean, std in zip(x, y_mean, y_std):
                if x >= x_limit:
                    print("Final: x=%d \t mean=%f \t std=%f"%(x, mean, std))
                    break
            print("Max: x=%d \t mean=%f \t std=%f"%(max_x, max_y_mean, max_y_std))
